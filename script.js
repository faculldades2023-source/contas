...
var DB={users:[],customBancos:{},userData:{}};
...
function authGetUsers(){return DB.users||[];}
function authSetUsers(arr){DB.users=arr;dbSave();}
...
function admDeletarUser(targetUser){
  ...
  authSetUsers(users);
  delete DB.userData[targetUser];dbSave();
  renderAdmUsers();renderLoginUsers();
}
...
function _dadosUsuario(){var u=window._currentUser||'guest';if(!DB.userData[u])DB.userData[u]={tx:[],bancos:['bb','nu','bradesco','renner','caju']};return DB.userData[u];}
function getBancosAtivos(){var d=_dadosUsuario();return d.bancos&&d.bancos.length?d.bancos:['bb','nu','bradesco','renner','caju'];}
function setBancosAtivos(arr){_dadosUsuario().bancos=arr;dbSave();}
function iniciarApp(){carregarBancosCustom();modoLocal=true;var d=_dadosUsuario();lista=d.tx||[];renderSidebarBancos();renderCards();renderBancoPicker();sincSidebar();setView('dash');render();}
