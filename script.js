...
/* ═══ BANCO DE DADOS REMOTO (Vercel Blob) ═══ */
var DB={customBancos:{},tx:[],bancos:['bb','nu','bradesco','renner','caju']};
var _dbSaveTimer=null;
async function dbLoad(){
  try{
    var r=await fetch('/api/db');
    var j=await r.json();
    if(j&&(j.tx||j.customBancos))DB=j;
  }catch(e){console.error('Erro ao carregar banco de dados remoto:',e);}
}
function dbSave(){
  ...
  _dbSaveTimer=setTimeout(function(){
    fetch('/api/db',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(DB)})
      .catch(function(e){console.error('Erro ao salvar no banco de dados remoto:',e);});
  },350);
}
...
function getBancosAtivos(){return DB.bancos&&DB.bancos.length?DB.bancos:['bb','nu','bradesco','renner','caju'];}
function setBancosAtivos(arr){DB.bancos=arr;dbSave();}
function iniciarApp(){carregarBancosCustom();lista=DB.tx||[];renderSidebarBancos();renderCards();renderBancoPicker();sincSidebar();setView('dash');render();}
...
function salvarBancosCustom(){var custom={};Object.keys(BANCOS).forEach(function(k){if(BANCOS[k]._custom)custom[k]=BANCOS[k];});DB.customBancos=custom;dbSave();}
function carregarBancosCustom(){var saved=DB.customBancos||{};Object.keys(saved).forEach(function(k){BANCOS[k]=saved[k];});}
...
var _pagId=null,_pagCartaoSel=null,_pagValorTotal=0,_pagJaPago=0,_pagModo='debito';
function _hojeISO(){var d=new Date();return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');}
...
function pagar(id){
  ...
  _pagId=id;_pagCartaoSel=null;_pagModo='debito';
...
function setPagModo(modo){
  _pagModo=modo;_pagCartaoSel=null;
  var bd=document.getElementById('pagModoDebito'),bc=document.getElementById('pagModoCredito');
  if(bd)bd.classList.toggle('on',modo==='debito');
  if(bc)bc.classList.toggle('on',modo==='credito');
  var lbl=document.getElementById('pagCartaoLabel');
  if(lbl)lbl.textContent=modo==='debito'?'Pagar com (débito)':'Usar crédito de';
  var hint=document.getElementById('pagCreditoHint');
  if(hint)hint.style.display=modo==='credito'?'block':'none';
  document.getElementById('pagInsuf').style.display='none';
  _renderCartoesPag();
...
function _renderCartoesPag(){
  ...
  container.innerHTML='';
  var tAtual=lista.find(function(x){return x.id==_pagId;});
  var bancoOrigem=tAtual?(tAtual.banco||'bb'):null;
  getBancosAtivos().forEach(function(k){
    var b=BANCOS[k];if(!b)return;
    if(_pagModo==='credito'&&k===bancoOrigem)return; // não pode usar o crédito do próprio cartão que está pagando
    ...
    if(_pagModo==='debito'){
      ...
      div.innerHTML='<div class="pag-card-ico" style="background:'+b.corLight+'">'+b.emoji+'</div>'
        +'<span class="pag-card-nome">'+b.nome+'</span>'
        +'<span class="pag-card-saldo" style="color:var(--mut)">💳 crédito</span>';
    }
...
function confirmarPagamento(){
  ...
  var quitado=novoPago>=_pagValorTotal-0.001;
  if(quitado){
  ...
  if(_pagModo==='credito'){
    var bOrigem=BANCOS[t.banco||'bb'];
    var novaDespesa={
      id:String(Date.now())+'c',
      tipo:'despesa',
      banco:_pagCartaoSel,
      desc:'Fatura '+(bOrigem?bOrigem.nome:'')+' — '+t.desc,
      valor:v,
      cat:t.cat||'outros',
      data:_hojeISO(),
      pago:false
    ...
    showToast(quitado?'✅ Conta quitada no crédito!':'💰 Parcial pago no crédito','#3b7ef8');
  }else{
    showToast(quitado?'✅ Conta quitada!':'💰 Parcial salvo — restam '+fmt(_pagValorTotal-novoPago),'#10b981');
  }
  fecharPagModal();
  salva(null,t,null);
}
...
function fecharPagModal(){
  ...
  _pagId=null;_pagCartaoSel=null;_pagValorTotal=0;_pagJaPago=0;_pagModo='debito';
}
...
window.addEventListener('DOMContentLoaded',async function(){
  ...
  document.addEventListener('keydown',function(e){if(e.key==='Escape'){fecharModal();fecharDel();fecharBancosModal();fecharPagModal();closeGearMenu();}});
